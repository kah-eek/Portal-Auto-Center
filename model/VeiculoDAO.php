...
class VeiculoDAO
{
  ...
  */
  function getImagensCarroByNomeParceiro($nomeParceiro)
  {
    ...
    $stmt = $con->prepare(
      ...
      '* '.
      'FROM view_imagem_veiculo_parceiro '.
      'WHERE id_tipo_veiculo = 1 AND nome_fantasia LIKE ?'
    );
  ...
  /**
  * Obtém todos as imagens das motos e suas informações entreladas a ela conforme o parceiro informado
  * @return Array Contendo todos as imagens e suas informações entreladas a ela na base de dados
  * Obs.: Caso ocorra algum erro ao tentar realizar a consulta na base de dados este retornará um array contendo um índice ("error") com o valor true ("error":true)
  */
  function getImagensMotoByNomeParceiro($nomeParceiro)
  {
    ...
    $stmt = $con->prepare(
      ...
      '* '.
      'FROM view_imagem_veiculo_parceiro '.
      'WHERE id_tipo_veiculo = 2 AND nome_fantasia LIKE ?'
    );
